...
namespace App\Plugin\WeChatPersonal\Service\Bind;
...
class Order implements \App\Plugin\WeChatPersonal\Service\Order
{
    /**
     * 微信赞赏码单笔限额（元）。这是微信的平台规则、不是站长可选项：
     * 超过这个数买家扫了也付不成，所以只能在下单时挡住。
     */
    private const REWARD_MAX_AMOUNT = 200.00;
    ...
    public function create(string $tradeNo, float $amount, string $type, string $returnUrl, string $notificationUrl, array $config = [], int $configId = 0): array
    {
        ...
        $this->assertPayable($type, $amount, $config, $configId);
        ...
        $order = DB::transaction(function () use ($type, $amount, $tradeNo, $returnUrl, $notificationUrl, $configId, $expire) {
            ...
            //金额撞车只在同一个账号内才有歧义。
            $payAmount = WeChatPersonal::available($type, $amount, $expire, $configId);

            //占位避让是按 0.01 递增的，可能把贴着上限的单顶过 200；按最终收款额再确认一次
            if ($type === 'reward') {
                $this->assertRewardLimit($payAmount);
            }

            $order = new WeChatPersonal();
        ...
        return ['trade_no' => $order->trade_no, 'url' => '/plugin/WeChatPersonal/order/pay?tradeNo=' . $order->trade_no];
    }
     ...
     */
    /**
     * 赞赏码限额校验：超限直接拒单，并告诉买家该换支付方式
     * @throws JSONException
     */
    private function assertRewardLimit(float $amount): void
    {
        if ($amount > self::REWARD_MAX_AMOUNT) {
            throw new JSONException(sprintf(
                "微信赞赏码单笔最高 %s 元，本单需支付 %s 元已超出，请选择其他支付方式",
                number_format(self::REWARD_MAX_AMOUNT, 0),
                number_format($amount, 2)
            ));
        }
    }

    private function assertPayable(string $type, float $amount, array $config, int $configId): void
    {
        ...
        if ($type === 'phone') {
            if (Settings::field($config, 'phone') === '') {
                throw new JSONException("手机号转账未配置收款手机号，请选择其他支付方式");
            }
            return;
        }

        //赞赏码页面是把图片原样贴出来让买家扫，不需要解析成链接；
        //普通收款码页面则要用解析出的支付链接在前端重绘二维码。
        if ($type === 'reward') {
            if (Settings::field($config, 'reward_url') === '') {
                throw new JSONException("该支付接口未配置赞赏码，请选择其他支付方式");
            }
            //赞赏码单笔上限是微信的平台规则，超了买家扫码也付不成；
            //与其让他跳进收银台白扫一场再回来找客服，不如下单时就说清楚（GitHub #899）
            $this->assertRewardLimit($amount);
            return;
        }

        if (Settings::qrUrl($configId, $config, 'qrcode', 'wx_url') === '') {
            ...
            throw new JSONException($hasImage
                ? "微信收款码图片无法识别，请换一张更清晰的图（裁掉多余背景只留二维码），或在支付接口配置里直接填写「收款链接」"
                : "该支付接口未上传微信收款码，请在支付插件的配置里上传");
        }
    }
     ...
     * 商城侧的验签（Pay\WeChatPersonalPay\Impl\Signature）就是按支付接口的配置档取 key 的，
     * 两边取不到同一个 key 就会「钱收了、货不发」。
     */
    private function notifyShop(WeChatPersonal $order, string $token): void
    {
        $post = [
