 ...
 * 配置访问层。
 *
 * 2.0 起收款信息（Token / 收款码 / 手机号）都在「支付插件 → 支付宝-接口(个人挂机版) → 配置」
 * 的配置档里，一套配置 = 一个支付宝收款账号 = 一台挂机手机。通用插件只留全局引擎参数。
 *
 * 1.x 升级上来的站点，老配置仍留在通用插件里，这里全部做回落读取，站长不动配置也能继续收款。
 */
final class Settings
{
    public const PLUGIN = 'AlipayPersonal';
    public const HANDLE = 'AlipayPersonalPay';
     ...
     * @param string $urlKey 解析结果字段名（alipay_url / reward_parsed_url）
     */
    public static function qrUrl(int $configId, array $profile, string $imageKey = 'qrcode', string $urlKey = 'alipay_url'): string
    {
        ...
        //配置档没配图：整套回落到通用插件的老配置（含它早就解析好的 alipay_url）
        if ($image === '') {
